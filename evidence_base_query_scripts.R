...
path <- 
data.study <- food.study
data.biblio <- food.biblio
data.interv <- food.interv
data.outcome <- food.outcome
data.biomes <- food.biomes
data.pathways <- food.pathways
...
pdf(file="mat_liv_std_Country_Map.pdf", width=16, height=8.5)
ggplot() + geom_map(data=countries_only, aes(map_id=Code, fill=counts),map=map) + geom_map(data=oecd, aes(map_id=CODE),fill="#f0f0f0",map=map) + expand_limits(x=map$long,y=map$lat) + theme(background="black") + scale_fill_gradient2(low="#d9f0a3",mid="#41ab5d",high="#004529",midpoint=47,limits=c(0,94))
dev.off()
...
par(mar = c(7, 4, 2, 2) + 0.2)
barplot(as.numeric(icounts),horiz=TRUE,xlim=c(0,100),names.arg=int_labels,las=1,xlab="Number of studies by IUCN-CMP intervention subcategories",cex.axis=1.5)
box()
dev.off()
...
par(mar = c(7, 4, 2, 2) + 0.2)
barplot(as.numeric(ocounts),horiz=TRUE,xlim=c(0,100),names.arg=out_labels,las=1,xlab="Number of studies by domain of human well-being",cex.axis=1.5)
box()
dev.off()
